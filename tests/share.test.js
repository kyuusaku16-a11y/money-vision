...
import { THRESHOLDS, judgeType, allTypes, buildShareText, buildAxisDetails, nearestEvolution } from '../src/share.js';
...
test('buildAxisDetails: 境目までの変化量（進化の条件）を出す', () => {
  const p = {
    annualIncome: 3000000, // 月収25万
    annualExpense: 2520000, // 月支出21万 → 余剰4万・貯蓄率16%
    totalAsset: 600000,
    investedAsset: 100000,
    monthlyInvest: 10000,
  };
  const d = buildAxisDetails(p);
  // 軸1: 貯蓄率20%には余剰5万必要 → あと月1万円
  assert.ok(d[0].flip.text.includes('1万円'));
  // 軸2: 投資30%には18万必要 → あと8万円
  assert.ok(d[1].flip.text.includes('8万円'));
  // 軸3: 6ヶ月分=126万、現金50万 → あと76万円
  assert.ok(d[2].flip.text.includes('76万円'));
  // 軸4: 強度50%には積立2万必要 → あと月1万円
  assert.ok(d[3].flip.text.includes('1万円'));
  // 進化先のタイプ名がついてくる（1文字だけ変わったコード）
  assert.equal(d[0].flip.code, 'CMLN');
  assert.ok(d[0].flip.name);
});

test('buildAxisDetails: しきい値を満たした軸に進化条件は出ない', () => {
  const p = {
    annualIncome: 4800000,
  ...
  const d = buildAxisDetails(p);
  assert.equal(d[0].flip, null);
  assert.equal(d[1].flip, null);
  assert.equal(d[2].flip, null); // 700/24≒29ヶ月 → S
  assert.equal(d[3].flip, null);
});
...
test('nearestEvolution: 境目にいちばん近い軸をひとつ選ぶ', () => {
  const p = {
    annualIncome: 3000000,
    annualExpense: 2520000,
    totalAsset: 600000,
    investedAsset: 100000,
    monthlyInvest: 19000, // 強度47.5% → 境目まで2.5%（4軸で最接近）
  };
  const evo = nearestEvolution(buildAxisDetails(p));
  assert.equal(evo.axisLabel, 'しこみ');
  assert.ok(evo.name); // 進化先タイプ名
  assert.ok(evo.text.includes('進化'));
});

test('nearestEvolution: 全軸そろっていたら null（完成形）', () => {
  const p = {
    annualIncome: 4800000,
    annualExpense: 2880000,
    totalAsset: 10000000,
    investedAsset: 3000000,
    monthlyInvest: 160000,
  };
  assert.equal(nearestEvolution(buildAxisDetails(p)), null);
});
